...
class OrderView {
    ...
    public function __construct($order, $mode, $suppressGraphics){
        $this->order = $order;
    ...
    private function getCustomerModeHeader() {
        ...
        $logoImg = Configuration::instance()->getInternalImageFullURL('images/logo.png');
        ...
        if (!$this->suppressGraphics) {
            ...
            $html .= "<img width='150px' src='$logoImg'/>";
            $html .= "</td>";
            }
    ...
    private function getRestaurantModeHeader() {
        ...
        $orderDescription = strtoupper($this->order->getDisplayOrderType()) . ' ORDER - ' . $this->order->getDisplayDeliveryDateTime();
        ...
        $html .= "<p style='text-align:center;font-size:18px;font-weight:bold;margin:0px'>$orderDescription</p>";
    ...
    private function getOrderHighlights() {
        ...
        $html .= "<tr>";
        $html .= "<td style='padding: 2px'>";
        $html .= $this->order->getDisplayOrderHeaderId();
        $html .= "</td>";
        $html .= "</tr>";
        ...
        $html .= "<tr>";
        $html .= "<td>";
        $html .= $this->order->getDisplayDeliveryDateTime();
        $html .= "</td>";
        $html .= "</tr>";
        ...
        $html .= "<tr>";
        $html .= "<td style='padding: 2px'>";
        $html .= "Please contact us at (860)&nbsp;284-4466 if you need any assistance with your order.";
        $html .= "</td>";
        $html .= "</tr>";

        $html .= $this->getEmptyRow();

        $html .= $this->getButton("IM HERE TO PICKUP",Configuration::instance()->getServerURL("notifyCustomerArrival?orderId=" . $this->order->getOrderHeaderId()));
        ...
        $html .= $this->getButton("JOIN NOW",Configuration::instance()->getServerURL("signup"));
    ...
    private function getNameValueTable($caption,$colInfo,$data) {
        ...
        foreach ($colInfo as $colName=>$colLabel) {
            $html .= "<tr style='padding:2px 0px 2px 0px;'>";
    ...
    private function getOrderSummary() {
        ...
        return $this->getNameValueTable("Order Summary",
                                ...
                                'total'=>'Total:',
                                'payment'=>'Payment:'],
                                ['orderId'=> $this->order->getDisplayOrderHeaderId(),
    ...
    private function getCustomerInfo() {
        ...
        return $this->getNameValueTable("Customer Information",
            ...
            ['customerName'=> $this->order->getDisplayCustomerName(),
             'customerPhone'=>$this->order->getCustomerCellPhone()
            ]);
    }
    ...
    private function getOrderitem($qty,$item,$price,$amount,$optionText,$instructions) {
        ...
        if (!empty($optionText)) {
            ...
            $html .= "&nbsp;";
            $html .= "</td>";
            $html .= "<td width='60%' style='font-size:80%;padding:2px 2px 2px 2px'>";
            $html .= "$optionText" ;
            $html .= "</td>";
            $html .= "<td align='right' width='12.5%' style='padding:2px 2px 2px 2px'>";
            $html .= "&nbsp;";
    ...
    private function getOrderItems() {
        ...
        foreach ($this->order->getItems() as $item) {
            $html .= $this->getOrderitem($item->getQty(),$item->getMenuItemCode(),$item->getDisplayPrice(),
    ...
    private function getOrderInstructions() {
        $header = $this->getColumnHeader("Special Instructions",1);

        $instructions = $this->order->getInstructions() ;
        if (empty($instructions))
            $instructions = 'N/A';
            ...
            echo
               "<div style='height: 100vh;position: relative;'>".
                "<div style='position: absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);width:35vw;box-shadow: 0px 0px 20px 5px rgba(0, 0, 0, .6);border: 1px solid black;'>" .
                  "<p style='color:red;text-align:center'> Invalid Order</p>".
                "</div>".
                "</div>";
        */
            displayInfoPage(ERROR_MESSAGE,'Invalid Order');
            return;
        }
        ...
        if($this->mode == 1)
            $orderHighlights = $this->getOrderHighlights();
        else
            $orderHighlights = '';
        ...
        $orderInstructions = $this->getOrderInstructions();
        ...
        $smarty->assign('highlights', $orderHighlights);
        $smarty->assign('customerInfo', $customerInfo);
        $smarty->assign('reservationSummary', '');
        $smarty->assign('orderSummary', $orderSummary);
        $smarty->assign('orderItems', $orderItems);
        $smarty->assign('instructions', $orderInstructions);
